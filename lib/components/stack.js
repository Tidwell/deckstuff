...
Stack.prototype.add = function(card, position) {
	...
	card.changeZone(this.zoneId);
	card.changeStack(this.id);
	this.events.emit('stack:added', card);
};

Stack.prototype.draw = function(position) {
	var card = this.drawCard(position);
	this.events.emit('stack:removed', card);
	return card;
}
...
Stack.prototype.shuffle = function() {
    ...
    this.events.emit('stack:shuffled', this);
    return this.cards;
};
